    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($resultado->num_rows > 0) {
            ...
            if ($resultado_rol->num_rows > 0) {
                ...
                if ($tipo_rol == 1) {
                    echo "<script>alert('Inicio de sesión correctamente como Guardian'); window.location.href = 'index.php?controller=general&accion=imagen_guardian';</script>";
                } elseif ($tipo_rol == 2) {
                    echo "<script>alert('Inicio de sesión correctamente como Directivo'); window.location.href = 'index.php?controller=general&accion=imagen_directivo';</script>";
                } elseif ($tipo_rol == 3) {
                    echo "<script>alert('Inicio de sesión correctamente como Secretario'); window.location.href = 'index.php?controller=general&accion=imagen_secretario';</script>";
                } elseif ($tipo_rol == 4) {
                    echo "<script>alert('Inicio de sesión correctamente como Instructor'); window.location.href = 'index.php?controller=general&accion=imagen_instructor';</script>";
                } else {
                    echo "<script>alert('Rol desconocido'); window.location.href = 'login.php';</script>";
                }
            } else {
                echo "<script>alert('Rol no encontrado'); window.location.href = 'index.php';</script>";
            }
        } else {
